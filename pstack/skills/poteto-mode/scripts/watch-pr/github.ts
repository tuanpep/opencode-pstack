...
function parseReviewThreadPage(value: unknown): {
  ...
  if (pageInfo.hasNextPage && !cursor?.trim())
  ...
  return { threads, endCursor: pageInfo.hasNextPage ? cursor : null };
}
function summarizeReviewThreads(threads: readonly RawReviewThread[]): readonly T.ReviewThread[] {
...
export function parseReviewThreads(pages: readonly unknown[]): readonly T.ReviewThread[] {
  const threads: RawReviewThread[] = [];
  ...
  for (const [index, value] of pages.entries()) {
    ...
    if (page.endCursor === null) {
      if (index !== pages.length - 1) missing("reviewThreads unexpected page after last page");
      return summarizeReviewThreads(threads);
    }
    if (page.endCursor === after || seen.has(page.endCursor))
      missing("reviewThreads.pageInfo.endCursor (repeated)", page.endCursor);
    seen.add(page.endCursor);
    after = page.endCursor;
  }
  missing("reviewThreads final page");
}
export async function readReviewThreads(
  queryPage: (after: string | null) => Promise<unknown>
): Promise<readonly T.ReviewThread[]> {
  const threads: RawReviewThread[] = [];
  const seen = new Set<string>();
  let after: string | null = null;
  while (true) {
    const page = parseReviewThreadPage(await queryPage(after));
    threads.push(...page.threads);
    if (page.endCursor === null) return summarizeReviewThreads(threads);
    if (page.endCursor === after || seen.has(page.endCursor))
      missing("reviewThreads.pageInfo.endCursor (repeated)", page.endCursor);
    seen.add(page.endCursor);
    after = page.endCursor;
...
export class GhGitHubReader implements T.GitHubReader {
  ...
  async reviewThreads(
    ...
    return readReviewThreads((after) => {
