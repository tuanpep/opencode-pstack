...
test('mergeConfig keeps a real API key', () => {
  const merged = mergeConfig(
    ...
    { provider: { openai: { options: { apiKey: '{env:OPENAI_API_KEY}' } } }, default_agent: 'fox' },
  )
  assert.equal(merged.provider.openai.options.apiKey, 'sk-live')
  assert.equal(merged.default_agent, 'fox')
})

test('applyConfig mutates the runtime config object', () => {
  const config = { plugin: ['other'] }
  applyConfig(config, { default_agent: 'fox', instructions: ['WORKFLOW.md'] })
  assert.equal(config.default_agent, 'fox')
...
test('installPlugins copies skills, agents, commands, and WORKFLOW.md', () => {
  ...
  try {
    ...
    assert.equal(existsSync(join(destination, 'agents', 'wolf.md')), true)
...
test('installPlugins removes stale owned files', () => {
  ...
  try {
    ...
    writeFileSync(join(destination, 'agents', 'code.md'), 'stale\n')
    writeFileSync(join(destination, 'agents', 'review.md'), 'stale\n')
    ...
    writeFileSync(join(destination, 'agents', 'deep.md'), 'stale\n')
    ...
    assert.equal(existsSync(join(destination, 'agents', 'code.md')), false)
    assert.equal(existsSync(join(destination, 'agents', 'review.md')), false)
    ...
    assert.equal(existsSync(join(destination, 'agents', 'deep.md')), false)
